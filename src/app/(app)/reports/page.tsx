...
export default async function ReportsPage() {
  ...
  return (
    <div>
      <Header
        title="Reports"
        subtitle={isManagement ? 'Management analytics and pipeline health' : 'Pipeline overview'}
        actions={
          <div className="flex items-center gap-2">
            {isManagement && (
              <Link href="/reports/management" className="inline-flex items-center gap-1.5 text-xs font-medium text-blue-600 border border-blue-200 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors">
                Management Review →
              </Link>
            )}
            {isManagement && <ExportButton />}
          </div>
        }
      />
