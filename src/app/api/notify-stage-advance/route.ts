...
async function buildFileContent(product_id: string, depts: string[]): Promise<{
  ...
  // re-generated from the parsed data, so the BOM team opens the same workbook
  ...
  for (const file of otherFiles) {
    ...
    if (!loc) {
      console.error('[notify-stage-advance] cannot resolve storage path for', file.file_url)
    ...
    try {
      ...
      if (error || !blob) {
        ...
        console.error('[notify-stage-advance] attachment download failed', loc.path, error?.message)
        skipped.push(file.name)
        continue
      }
      if (blob.size > budget) {
        console.warn('[notify-stage-advance] attachment over size budget, skipped', file.name, blob.size)
        skipped.push(file.name)
        continue
      }
      budget -= blob.size
      ...
      console.error('[notify-stage-advance] attachment error', loc.path, err)
      skipped.push(file.name)
    }
  }

  // Name the attached documents. Previously this block only rendered when there
  // were images, so an email carrying just the merchandising Excel said nothing
  // about it and looked like the sheet had been left out.
